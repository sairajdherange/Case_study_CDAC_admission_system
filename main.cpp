					...
					for(unsigned x = 0 ; x < students_v.size() ; x++ )
					{
						if(form_no == students_v[x].get_form_no() && pass == students_v[x].get_name())
						{
							...
							while((choice9 = system1.student_menu2()) != 0)
							{
								...
								break;
								case 4: {
									cout << "The allocated center is ";
									for(unsigned j = 0 ; j < centers_v.size() ; j++)
										if(centers_v[j].id == students_v[x].allocated_center_id)
											cout << centers_v[j].name << ", " << centers_v[j].address << " for " << students_v[x].allocated_course_name << " course." << endl;
								}
								break;
								case 5:
								{
